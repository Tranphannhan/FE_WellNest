...
import ViewParaclinicalResults, {
  NormalTestResult,
} from "./CreateResultsComponent/ViewParaclinicalResults";
...
import {
  CheckPrescription,
  confirmCompletion,
  fetchMedicalExaminationCardDetail,
  getExaminationResults,
  getResultsByRequestTesting,
  latestDiagnosis,
  getDoctorTemporaryTypes,
  checkCanLamSangByDepartmentId,
} from "@/app/services/DoctorSevices";
import {
  generateTestResultsType,
...
export default function Patient() {
  ...
  const [showPrescriptionPopup, setShowPrescriptionPopup] =
  ...
  const [showModalCompletion, setShowModalCompletion] =
    useState<boolean>(false);
  const [testResults, setTestResults] = useState<NormalTestResult[]>([]);
  const [allowsGeneratingResults, setAllowsGeneratingResults] =
    useState<boolean>(false);
  const [hiddenButton, setHiddenButton] = useState<boolean>(false);
  const [hasPendingExamRequest, setHasPendingExamRequest] =
    useState<boolean>(false); // New state for pending exam requests
  const router = useRouter();
  const { id } = useParams();
  const searchParams = useSearchParams();
  const WaitClinicalExamination =
    searchParams.get("WaitClinicalExamination") === "true";
  ...
  const checkPrescription = async () => {
    ...
    if (
      resCheckPrescription &&
      resCheckPrescription.data?.data?.TrangThai === "DaXacNhan"
    ) {
      setBeFinished(true);
    }
  };
  ...
  const [showExaminationRequestPopup, setShowExaminationRequestPopup] =
    useState(false);
  ...
  async function getValueRender() {
    ...
    if (token) {
      try {
        const decoded = jwtDecode<{ _ID_Khoa: { _id: string } }>(token);
        const resCanLamSang = await checkCanLamSangByDepartmentId(decoded._ID_Khoa._id)
        setTesting(resCanLamSang)
      } catch (err) {
        console.error("Token không hợp lệ:", err);
      }
    }
    const res: MedicalExaminationCard | null =
      await fetchMedicalExaminationCardDetail(id as string);
    if (res) {
      console.log(res);
  ...
  const [
    continueRenderExaminationResults,
    setContinueRenderExaminationResults,
  ] = useState<boolean>(false);
  const checkRenderContinueExaminationResults = async () => {
    const data: generateTestResultsType | null = await getExaminationResults(
      id as string
    );
  ...
  const checkPendingExaminationRequests = async () => {
    ...
    if (examRequests && Array.isArray(examRequests)) {
      const hasPending = examRequests.some(
        (request) =>
          request.TrangThai === false && request.TrangThaiThanhToan === true
      );
          ...
          tabbarItems: [
            {
              ...
              link: `/Doctor/Patient/ToExamine/${id}${WaitClinicalExamination ? "?WaitClinicalExamination=true" : ""}`,
            },
            {
              text: "Tạo kết quả",
              link: `/Doctor/Patient/ToExamine/${id}/CreateResults`,
            },
          ],
        }}
      />
        ...
        <div className="CreateResults-redirectFrame__actionButtonsContainer">
          <div className="CreateResults-redirectFrame__actionButtonsContainer__leftButtons">
            <button
              ...
              disabled={
                !continueRenderExaminationResults || isPrescriptionCreating
              }
              style={{
                borderRadius: "8px",
                cursor:
            ...
            <button
              ...
              disabled={
                ...
                hasPendingExamRequest
              } // Add hasPendingExamRequest to disabled condition
              style={
                !continueRender ||
                allowsGeneratingResults ||
                hasPendingExamRequest
      ...
      <div className="CreateResults-bodyFrame">
        <div className="CreateResults-bodyFrame__navigationBar">
          <Diagnosiscomponent
            ...
            page={page}
            reload={() => {
              setprescriptionComponentKey((prev) => prev + 1);
            }}
          />
        </div>

        <div>
          {page === "Chuẩn đoán sơ bộ" && (
            <DiagnosisComponent
              reLoad={() => {
                checkRender();
              }}
