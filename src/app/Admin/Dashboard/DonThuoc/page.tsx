...
import { PrescriptionStatsType } from "@/app/types/hospitalTypes/hospitalType";
import { fetchPrescriptionsByDateRange } from "@/app/services/FileTam";
import {
  Paper,
  TextField,
  Typography,
...
const PrescriptionTransactions: React.FC = () => {
  ...
  const fetchPrescriptionTransactions = useCallback(async () => {
    try {
      ...
      const prescriptionResponse = await fetchPrescriptionsByDateRange(fromDate, toDate, year).catch(() => ({
        data: [],
      }));
      prescriptions = prescriptionResponse.data || [];

      const transactions: Transaction[] = prescriptions
      ...
      setPrescriptionTransactions(transactions);
      setCurrentPage(1);
    } catch (error) {
      console.error("Error fetching prescription transactions:", error);
      setPrescriptionTransactions([]);
    }
  ...
  const fetchAllPrescriptions = useCallback(async () => {
    try {
      ...
      // Call fetchPrescriptionsByDateRange with all=true to include both paid and unpaid, but with filters
      const response = await fetchPrescriptionsByDateRange(fromDate, toDate, year, true).catch(() => ({
        data: [],
      }));
      prescriptions = response.data || [];

      const paid = prescriptions.filter((p) => p.TrangThaiThanhToan === true).length;
      const unpaid = prescriptions.filter((p) => p.TrangThaiThanhToan === false).length;

      setPaidCount(paid);
      setUnpaidCount(unpaid);
    } catch (error) {
      console.error("Error fetching all prescriptions for pie chart:", error);
      setPaidCount(0);
      setUnpaidCount(0);
    }
  }, [filterStartDate, filterEndDate, filterMonth, filterQuarter, filterYear]);
  ...
  const sortedAndFilteredTransactions = prescriptionTransactions
    .filter((transaction) =>
  ...
  const totalPrescriptionRevenue = sortedAndFilteredTransactions.reduce((sum, t) => sum + t.amount, 0);
  ...
  return (
    <div className="AdminContent-Container">
      ...
      <div className="summary-cards mb-4">
        {[
          { title: "Tổng Tiền", amount: calculateTotalByYear() || totalPrescriptionRevenue },
          { title: "Theo Ngày", amount: calculateTotalByDateRange() },
          { title: "Theo Tháng", amount: calculateTotalByMonth() },
        ].map((card, i) => (
          <div key={i} className={`summary-card summary-card-${i}`}>
            <Paper
              sx={{
                p: 3,
                textAlign: "left",
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
        ...
        <PaymentPieChart
          title="Tỷ lệ Đơn Thuốc Đã / Chưa Thanh Toán"
          data={pieChartData}
      ...
      <TransactionTable
      title="Danh sách hóa đơn thuốc"
      headCells={headCells}
      transactionsToShow={transactionsToShow}
      totalAmount={totalPrescriptionRevenue}
      searchTerm={searchTerm}
      orderBy={orderBy}
      order={order}
      handleInputChange={handleInputChange}
      handleRequestSort={handleRequestSort}
      emptyText="Không có hóa đơn thuốc để hiển thị."
    />
