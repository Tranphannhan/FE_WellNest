...
import { TestRequestType } from "@/app/types/hospitalTypes/hospitalType";
import { fetchTestRequestsByDateRange } from "@/app/services/FileTam";
import {
    Paper,
    TextField,
    Typography,
...
const TestRequestTransactions: React.FC = () => {
    ...
    const fetchTestRequestTransactions = useCallback(async () => {
        try {
            ...
            const testRequestResponse = await fetchTestRequestsByDateRange(fromDate, toDate, year).catch(() => ({
                data: [],
            }));
            testRequests = testRequestResponse.data || [];

            const transactions: Transaction[] = testRequests
            ...
            setTestRequestTransactions(transactions);
            setCurrentPage(1);
        } catch (error) {
            console.error("Error fetching test request transactions:", error);
            setTestRequestTransactions([]);
        }
    ...
    const fetchAllTestRequests = useCallback(async () => {
        try {
            ...
            const response = await fetchTestRequestsByDateRange(fromDate, toDate, year, true).catch(() => ({
                data: [],
            }));
            testRequests = response.data || [];

            const paid = testRequests.filter((t) => t.TrangThaiThanhToan === true).length;
            const unpaid = testRequests.filter((t) => t.TrangThaiThanhToan === false).length;

            setPaidCount(paid);
            setUnpaidCount(unpaid);
        } catch (error) {
            console.error("Error fetching all test requests for pie chart:", error);
            setPaidCount(0);
            setUnpaidCount(0);
        }
    }, [filterStartDate, filterEndDate, filterMonth, filterQuarter, filterYear]);
    ...
    const sortedAndFilteredTransactions = testRequestTransactions
        .filter((transaction) =>
    ...
    const calculateTotalByYear = () => {
        ...
        return totalTestRequestRevenue;
    };
    ...
    return (
        <div className="AdminContent-Container">
            <Typography variant="h5" align="center" sx={{ fontWeight: "bold", mb: 5 }}>
                Danh Sách Hóa Đơn Xét Nghiệm
            </Typography>
            ...
            <div className="summary-cards mb-4">
                {[
                    { title: "Tổng Tiền", amount: calculateTotalByYear() || totalTestRequestRevenue },
            ...
            <Typography variant="h6" align="center" sx={{ fontWeight: "bold", mb: 2 }}>
                Doanh Thu Xét Nghiệm Theo Ngày
            </Typography>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
                ...
                <PaymentPieChart
                    title="Tỷ lệ Xét Nghiệm Đã / Chưa Thanh Toán"
                    data={pieChartData}
            ...
            <TransactionTable
                ...
                totalAmount={totalTestRequestRevenue}
