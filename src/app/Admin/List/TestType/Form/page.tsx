...
function AddTestForm() {
  ...
  const checkTestName = async (tenXetNghiem: string): Promise<boolean> => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/Loaixetnghiem/CheckTestName`,
        {
          method: "POST",
      ...
      if (!response.ok) {
        console.error(
          `Check test name API error: Status ${response.status}, ${response.statusText}`
        );
        return false;
      }
  ...
  const validateForm = async () => {
    ...
    if (!loaiKetQua) {
      newErrors.loaiKetQua = "Vui lòng chọn loại kết quả.";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage(
          `Thêm thất bại: ${errorData.message || "Lỗi không xác định"}`
        );
      }
    } catch (error) {
              ...
              >
                ...
                >
                  <TextField
                    ...
                    value={maXetNghiem}
                    onChange={(e) => {
                      setMaXetNghiem(e.target.value);
                      setErrors((prev) => ({
                        ...prev,
                        maXetNghiem: undefined,
                      }));
                    }}
                    required
                  ...
                  <FormControl
                    sx={{ width: { sm: "60%" } }}
                    error={!!errors.loaiKetQua}
                  >
                    <InputLabel id="loai-ket-qua-label">
                      Loại Kết Quả *
                    </InputLabel>
                    <Select
                      ...
                      required
                    >
                      <MenuItem value="">Chọn loại kết quả</MenuItem>
                      <MenuItem value="DinhTinh">Định tính (Âm/Dương)</MenuItem>
                      <MenuItem value="DinhLuong">
                        Định lượng (có số & đơn vị)
                      </MenuItem>
                      <MenuItem value="HinhAnh">
                        Hình ảnh (X-quang, MRI...)
                      </MenuItem>
                      <MenuItem value="MoTa">Mô tả (Vi sinh, GPB...)</MenuItem>
                    </Select>
                    {errors.loaiKetQua && (
                      <FormHelperText error>{errors.loaiKetQua}</FormHelperText>
                    )}
                  </FormControl>
                </Box>
