import { showToast, ToastType } from "../lib/Toast";

const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL;


// lấy danh sách chờ xét nghiệm
export async function getWaitingForTest (id : string , page : number) {
    try{
        const respone = await fetch(`${API_BASE_URL}/Yeu_Cau_Xet_Nghiem/GetById_PhongTB_date/Pagination/?Id_PhongThietBi=${id}&TrangThai=false&page=${page}`);
        if(respone.ok){
        return (await respone.json())
        }else{
        console.error("Không tìm thấy danh sách chờ xét nghiệm");
    }
    }catch(error){
        console.error("Lỗi khi lấy danh sách chờ xét nghiệm", error);
        throw error; 
    }
}



// lấy chi tiết chờ xét nghiệm 
export async function getWaitingForTestDetail (id : string) {
  try {
    const response = await fetch(`${API_BASE_URL}/Yeu_Cau_Xet_Nghiem/YeuCauXetNghiemThuNgan/Detail?Id_PhieuKhamBenh=${id}&TrangThaiThanhToan=false`);
    if (!response.ok) return null;
    const data =await response.json();
    return data;
  } catch (error) {
    console.error("Lỗi khi lấy chi tiết danh sách chờ xét nghiệm", error);
    return null;
  }  
}



// Tạo kết quả xét nghiệm
export async function generateTestResults (fullForm){
     try {
        const formData = new FormData();
        for (const key in fullForm) {
            const value = fullForm[key as keyof typeof fullForm];
            if (value) formData.append(key, value);
        }

        const response = await fetch(`http://localhost:5000/Ket_Qua_Xet_Nghiem/Add`, {
            method: 'POST',
            body: formData,
        });

        if (!response.ok) {
            console.log('lỗi');
            
        }

        const data = await response.json();
        showToast ('Tạo thành công', ToastType.success);
        return data;
    } catch (error) {
        console.error("Fetch lỗi:", error);
        return null;
    }
}